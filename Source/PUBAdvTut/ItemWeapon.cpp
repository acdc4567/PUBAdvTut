...
void AItemWeapon::OnConstruction(const FTransform& Transform){
    ...
    SkeletalMesh->SetSkeletalMesh(ItemWeaponRow->SkeletalMesh,1);
...
void AItemWeapon::ReloadClip(){
    ...
    if(MagAccActorx1){
        ReloadRate=MagAccActorx1->ItemWeaponAccRow->ClipRate*ItemWeaponRow->ReplaceClipTime;
    }
    else{
        ReloadRate=ItemWeaponRow->ReplaceClipTime;
    }
    if(CharacterRef->GetIsProne()){
        ReloadRate=MyGameInstanceRef->ReplaceClipTime_Prone/ReloadRate;
    }
    else{
        ReloadRate=MyGameInstanceRef->ReplaceClipTime_Stand/ReloadRate;
    }
